    ...
    while True:
      ...
      for name, camera_process in self.camera_processes.items():
        ...
        if not process.is_alive():
          ...
          process = mp.Process(target=track_camera,
                               args=(name, self.config[name], FFMPEG_DEFAULT_CONFIG, GLOBAL_OBJECT_CONFIG,
                                     self.tflite_process.detection_queue, self.tracked_objects_queue,
                                     camera_process['fps'], camera_process['skipped_fps'],
                                     camera_process['detection_fps']))
          process.daemon = True
...
def main():
  ...
  for name, config in CONFIG['cameras'].items():
    ...
    camera_process = mp.Process(target=track_camera, args=(name, config, FFMPEG_DEFAULT_CONFIG, GLOBAL_OBJECT_CONFIG,
                                                           ...
                                                           camera_processes[name]['fps'],
                                                           camera_processes[name]['skipped_fps'],
